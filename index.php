...
// Fetch up to 8 featured/active products
$stmt = $pdo->query('SELECT p.id, p.title, p.price, p.image_path, p.stock FROM products p WHERE p.status = "active" ORDER BY p.created_at DESC LIMIT 8');
$products = $stmt->fetchAll();
// Fetch up to 4 sellers for the homepage
$sellers = $pdo->query("SELECT username FROM users WHERE role = 'seller' ORDER BY created_at DESC LIMIT 4")->fetchAll();
...
<body>
    <div class="top-nav">
        ...
        <div class="user-actions">
            <?php if (isset($_SESSION['user_id']) && isset($_SESSION['username'])): ?>
                <span style="margin-right:16px;font-weight:600;"><i class="fas fa-user"></i> <?= htmlspecialchars($_SESSION['username']) ?></span>
    ...
    <nav class="main-nav">
        ...
        <div class="search-box">
            <form action="/products.php" method="GET" class="search-bar-form">
                <input type="text" name="search" placeholder="Search products..." class="search-bar-input">
            ...
            <div class="promo-section">
                ...
                <div class="promo-grid">
                    <?php if (count($products) === 0): ?>
                        <div style="grid-column: 1/-1; text-align: center; color: #888; font-size: 1.2em;">No products available.</div>
                    <?php else: ?>
                        <?php foreach ($products as $product): ?>
                        <div class="promo-card">
                            <div class="promo-img" style="background: #f5f5f5; display: flex; align-items: center; justify-content: center; height: 160px;">
                                <img src="<?= htmlspecialchars($product['image_path']) ?>" alt="<?= htmlspecialchars($product['title']) ?>" style="max-width: 100%; max-height: 100%; object-fit: contain;">
                            </div>
                            <div class="promo-content">
                                <h3 class="promo-title" style="height:auto;min-height:40px;overflow:hidden;"><?= htmlspecialchars($product['title']) ?></h3>
                                <div class="promo-price">R<?= number_format($product['price'], 2) ?></div>
                                <div style="margin: 10px 0; color: #888; font-size: 0.95em;">
                                    <?= $product['stock'] > 0 ? 'In Stock' : 'Out of Stock' ?>
                                </div>
                                <form action="cart/add.php" method="POST" style="margin:0;">
                                    <input type="hidden" name="product_id" value="<?= $product['id'] ?>">
                                    <input type="hidden" name="quantity" value="1">
                                    <button type="submit" class="pay-btn" style="width:100%;padding:10px 0;font-size:1rem;" <?= $product['stock'] > 0 ? '' : 'disabled style="opacity:0.6;cursor:not-allowed;"' ?>>
            ...
            <div class="promo-section">
                ...
                <div class="promo-grid">
                    <?php if (count($sellers) === 0): ?>
                        <div style="grid-column: 1/-1; text-align: center; color: #888; font-size: 1.2em;">No sellers found.</div>
                    <?php else: ?>
